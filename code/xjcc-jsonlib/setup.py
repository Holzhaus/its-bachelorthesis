...
class BuildJarCommand(setuptools.Command):
    ...
    def run(self):
        ...
        buildroot = os.path.join(os.path.dirname(__file__),
                                 'xjcc_jsonlib', 'jsonlib')

        infiles = [
            os.path.join(buildroot, filename)
            for filename in [
                'pom.xml',
                'src/main/java/de/rub/xjcc/jsonlib/App.java',
                'src/test/java/de/rub/xjcc/jsonlib/AppTest.java',
            ]
        ]
        outfile = os.path.join(buildroot, 'target/jsonlib.jar')
...
setuptools.setup(
    ...
    package_data={
        'xjcc_jsonlib': ['jsonlib/target/jsonlib.jar'],
    },
    entry_points={
        'xjcc.converters': [
            'jsonlib = xjcc_jsonlib:JsonlibPlugin',
        ],
    },
    cmdclass={
