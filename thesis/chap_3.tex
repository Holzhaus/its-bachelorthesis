...
Es muss zudem sichergestellt sein, dass bei der Konversion auch tatsächlich valides \acrshort{json} bzw. wohlgeformtes \acrshort{xml} zurückgegeben wird. Daher werden beide Ausgabedokumente analysiert und auf Korrektheit überprüft.

Für \acrshort{json}-Daten wurde dazu das Python-Modul \emph{demjson} eingesetzt, das über ein umfangreiches Analyseprogramm (einen sogenannten \emph{Linter}) verfügt. Die Wohlgeformtheit der \acrshort{xml}-Daten wurde durch das Parsing mittels \emph{lxml} sichergestellt.
